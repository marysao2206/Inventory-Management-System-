...
export class AlignSchemaToDbml1711000006000 implements MigrationInterface {
  name = "AlignSchemaToDbml1711000006000";
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    if (!(await queryRunner.hasColumn("products", "created_by"))) {
      ...
      await queryRunner.query(
        "ALTER TABLE products ADD CONSTRAINT fk_users_id_products FOREIGN KEY (created_by) REFERENCES users(id)"
      );
    }
    ...
    `);
    await queryRunner.query(
      "ALTER TABLE inventory_items ADD CONSTRAINT fk_inventory_items_products FOREIGN KEY (product_id) REFERENCES products(id)"
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    `);
    await queryRunner.query(
      "ALTER TABLE inventory_items MODIFY status ENUM('in_stock','low_stock','out_of_stock','damaged') NOT NULL DEFAULT 'out_of_stock'"
    );
    ...
    await queryRunner.query(
      "ALTER TABLE inventory_items ADD CONSTRAINT fk_inventory_items_products FOREIGN KEY (product_id) REFERENCES products(id) ON DELETE CASCADE"
    );
