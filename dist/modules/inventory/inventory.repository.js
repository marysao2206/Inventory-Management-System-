...
class InventoryRepository {
    constructor() {
        ...
        this.logs = AppDataSource.getRepository(InventoryLog);
        this.details = AppDataSource.getRepository(InventoryDetail);
        this.stocks = AppDataSource.getRepository(Stock);
    }
    listItems() {
    ...
    async receiveStock(payload, userId) {
        return AppDataSource.transaction(async (manager) => {
            ...
            const detail = await details.save(details.create({
                inventoryItemId: savedItem.id,
                productId: payload.productId,
                userId: userId ?? null
            }));
            const stock = await stocks.save(stocks.create({
                detailId: detail.id,
            ...
            const log = await logs.save(logs.create({
                inventoryItemId: savedItem.id,
                actionType: "STOCK_IN",
                quantity: payload.quantity,
